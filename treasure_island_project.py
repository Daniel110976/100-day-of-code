...
if direction == "left":
    ...
    action = input("Do you want to fight or run? fight or run: ").lower()
    if action == "fight":
...
# choose right path
    ...
    action = input("Do you want to swim or build a raft? swim or raft: ").lower()
    if action == "raft":
        print("You build a raft and float down the river.")
        # Step 3: Choose a door
        door = input("You arrive at the entrance of a cave. Do you want to enter? yes or no: ").lower()
        if door == "yes":
            print("You enter the cave and find Three Doors. door 1 = red and door 2 = blue and door 3 = green.")
            door_choice = input("Which door do you choose? red, blue or green: ").lower()
